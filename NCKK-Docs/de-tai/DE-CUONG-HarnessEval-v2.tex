...
Table~\ref{tab:characteristics} presents a focused comparison of the five most closely related works against the six core characteristics of harness evaluation, following the format of Ho-Van~(2013). Each row represents a paper that partially addresses at least one harness-related concern; each column represents a characteristic directly tied to our research objectives. The table reveals that while individual characteristics have been partially investigated, \textbf{no prior work addresses more than two}, and \textbf{three characteristics remain entirely unexplored}.

\begin{table}[H]
\centering
\caption{Summary on Characteristics Investigated by Closest Related Works}
\label{tab:characteristics}
\begin{tabular}{l c c c c c c}
\toprule
\textbf{Reference} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} & \textbf{C5} & \textbf{C6} \\
& \rotatebox{70}{\small Tool Dispatch} & \rotatebox{70}{\small Context Eval} & \rotatebox{70}{\small Cross-Backend} & \rotatebox{70}{\small Ablation} & \rotatebox{70}{\small H/LLM Decomp.} & \rotatebox{70}{\small Multi-Metric} \\
\midrule
Bui~et~al.~\cite{A1} (OpenDev)       & $\triangle$ & $\triangle$ &             &             &             &             \\[3pt]
Lou~et~al.~\cite{A2} (AutoHarness)   & $\triangle$ &             &             & $\triangle$ &             &             \\[3pt]
Qin~et~al.~\cite{F2} (ToolLLM)       & $\checkmark^{\dagger}$ &  &             &             &             &             \\[3pt]
Chhikara~et~al.~\cite{E1} (Mem0)     &             & $\checkmark^{*}$ &          &             &             &             \\[3pt]
Ma~et~al.~\cite{G3} (AgentBoard)     & $\triangle$ & $\triangle$ &             &             &             & $\checkmark^{\ddagger}$ \\[3pt]
\midrule
\textbf{Ours (HarnessEval)}           & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
\bottomrule
\multicolumn{7}{l}{\small $\checkmark$ = Addressed as primary contribution \quad $\triangle$ = Partially addressed} \\
\multicolumn{7}{l}{\small $^{\dagger}$~Tool accuracy evaluated in isolation (API benchmarks), not within a coding harness} \\
\multicolumn{7}{l}{\small $^{*}$~Context evaluated on chatbot benchmarks (LoCoMo), not on coding tasks} \\
\multicolumn{7}{l}{\small $^{\ddagger}$~Multi-metric for agent \textit{output} (progress rate), not for \textit{harness infrastructure}} \\
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Three columns are entirely empty (C3, C4, C5).} No prior work evaluates cross-backend portability, conducts component ablation on coding harnesses, or decomposes harness vs.\ LLM contributions. These represent the core research gap.

    \item \textbf{Tool evaluation exists but outside harness context (C1).} ToolLLM~\cite{F2} evaluates tool selection accuracy on 16,000+ standalone APIs~($\checkmark^{\dagger}$), not within an integrated coding harness where tool dispatch interacts with context management. OpenDev~\cite{A1} describes its tool system but does not benchmark dispatch efficiency with formal metrics~($\triangle$).

    \item \textbf{Context evaluation exists but in the wrong domain (C2).} Mem0~\cite{E1} achieves 91\% latency reduction for persistent memory~($\checkmark^{*}$), but evaluates on the LoCoMo chatbot benchmark---not on coding tasks where context includes AST structures, dependency graphs, and test histories.

    \item \textbf{Multi-metric frameworks evaluate output, not harness (C6).} AgentBoard~\cite{G3} provides multi-dimensional evaluation via progress rates~($\checkmark^{\ddagger}$), but measures \textit{agent output quality}, not \textit{harness infrastructure quality}.

    \item \textbf{HarnessEval is the first to address all six characteristics,} bridging isolated component evaluations with a unified, ablation-driven, multi-backend framework for coding agent harness assessment.
\end{enumerate}
